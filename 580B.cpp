...
#define vp_in(x) for(auto &i:x) cin>>i.fi>>i.se
...
ll t,n,i,j=-1,k,a,b,ans;
void dihan()
{
	cin>>n>>k; vpll z(n); vll x(n),y(n); vp_in(z); srt(z);
    _for(i,0,n) x[i]=z[i].fi,y[i]=z[i].se;   pre_sum(y); 
    for_(i,n-1,-1) j=ub(x,x[i]-k),ans=max(ans,y[i]-(j?y[j-1]:0));
    print(ans);                    
}
int main(){____ dihan();}
