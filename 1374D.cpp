...
void dihan()
{
    cin>>n>>k; v_in(x,n); ans=0,a=0;

    _for(i,0,n) x[i]=k*ceil(1.0*x[i]/k)-x[i];
    srt(x);
    
    _for(i,1,n)
    {
    	if(x[i]>0&x[i]==x[i-1]) a++; else a=0;
    	if(x[i]+a*k>ans) ans=x[i]+a*k;
    }
   if(n==1&x[0]>ans) ans=x[0];  
   if(ans>0) out(ans+1); else out(0);
}
