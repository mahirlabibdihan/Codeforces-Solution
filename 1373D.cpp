...
void dihan()
{
    cin>>n; v_in(x,n); vll y,z,p,q; ll a=0,b=0,i,j,k;

    _for(i,0,n) if(i%2) z.pb(x[i]); else y.pb(x[i]);

   
    sum(y,s);

    for(i=0,j=0;j<z.size()&i<y.size();i++,j++)
      {
      	a+=y[i]; b+=z[j];
     ...
     for(i=1,j=0;j<z.size()&i<y.size();i++,j++)
      {
      	a+=y[i]; b+=z[j];
...
j=0; k=0;
   if(p.size()) j=*maxof(p);if(q.size())  k=*maxof(q);
   
   if(j>k)
   {
   	 i=*min_of(p.bg,maxof(p));
   	 if(i<0) j-=i;

     if(j>0) out(s+j);  else out(s);
   }

  
  else
  {
  	 i=*min_of(q.bg,maxof(q));
 
   	 if(i<0) k-=i;

    if(k>0) out(s+k);  else out(s);
  }
