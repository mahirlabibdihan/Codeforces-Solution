...
#define v2_in(x,n,m) vvi x(n,vi(m));_for(i,0,n) for(auto &j:x[i]) cin>>j
#define v2_out(x) _for(i,0,x.sz) {for(auto j:x[i]) cout<<j<<" "; cout<<endl;}
#define MAX 200001
ll t,n,i,j,k,a,b,c,m,l,r,p,q,s;
void dihan()
{
    cin>>n; v_in(x,n); vpll y; si z;

    _for(i,0,n) y.pb({x[i],i}); srt(y);
    _for(i,0,n-1) s+=y[i].fi;
   	_for(j,0,n-1) if(y[j].fi==s-y[n-1].fi)  z.insert(y[j].se);
    	
    if(s-y[n-2].fi==y[n-2].fi) z.insert(y[n-1].se);
    outn(z.sz); visit(z) cout<<i+1<<" ";
}
