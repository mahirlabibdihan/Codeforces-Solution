...
void dihan()
{
   str_in(x); cin>>k; n=x.len;
   _for(i,0,n) (x[i]!='?'&x[i]!='*')?m++:(x[i]=='?')?a++:b++;
   if(k>m) 
   {
   	 if(b==0) outn("Impossible");
   	 else
   	 	_for(i,0,n)
   	 	if(x[i]!='?'&x[i]!='*') cout<<x[i];
   	 	eif(x[i]=='*'&k>m)
   	 	{
           _for(j,0,k-m) cout<<x[i-1];
           m=k;
   	 	}   	 
   }
   else	if(m-k>a+b) outn("Impossible");
   else
   	 	_for(i,0,n)   	 	
           if(x[i]!='?'&x[i]!='*') 
           	if(x[i+1]!='?'&x[i+1]!='*') cout<<x[i];
            eif(m==k) cout<<x[i];
           eif(x[i+1]=='?'|x[i+1]=='*'&m>k)m--;
}
