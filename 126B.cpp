...
#define pstv(x) (x<0)?0:x
#define ngtv(x) (x>0)?0:x
#define out(a) cout<<a<<"\n"
...
int t,n,i,j,k,a,b,c,m,l,r,p,q,s;   string pat,x="Just a legend";  vi lps(MAX);
...
void dihan()
{
    cin>>pat; n=pat.len; LPS(); //v_outn(lps);
  
    _for(i,1,n-1) if(lps[i]>0&lps[i]==lps[n-1]) break; 
    if(i<n-1) _for(i,0,lps[n-1]) cout<<pat[i];
    else  if(lps[(pstv(lps[n-1]-1))]) _for(i,0,lps[lps[n-1]-1]) cout<<pat[i];  
    else  outn(x);    	
}
