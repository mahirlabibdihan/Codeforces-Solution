...
#define vs_out(x) visit(x) cout<<i<<endl
...
ll t,n,i,j,k,a,b,c,d,m,l,r,p,q,s,ans; vvll adj(100); vll visited(100);
...
void input(int m){while(m--){ll a,b,c;cin>>a>>b;adj[a].push_back(b);adj[b].push_back(a);}}
...
void dfs(vs &x,ll i,ll j)
{
    ...
    eif(x[i+1][j]=='.'&x[i-1][j]=='.'&x[i][j+1]=='*')  {outn(4);dfs(x,i,j+1);}

    eif(x[i+1][j]=='.'&x[i-1][j]=='.'&x[i][j-1]=='*')  {outn(5);dfs(x,i,j-1);}


    eif(x[i][j+1]=='.'&x[i][j-1]=='.'&x[i+1][j]=='*') 
    {outn(3);
      dfs(x,i+1,j);
      outn(6);
...
void dihan()
{
  cin>>n>>m; vs_in(x,n); vpll y;

  f(i,0,n)
   f(j,0,n)
    if(x[i][j]=='*') {dfs(x,i,j); break;}
