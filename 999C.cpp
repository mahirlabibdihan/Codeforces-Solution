...
void dihan()
{
    cin>>n>>k; str_in(x); vvi y(124); vi z;

    _for(i,0,n)  y[x[i]].pb(i);
    _for(a,97,124)
      	if(k>0&y[a].sz<=k) k-=y[a].sz; 
        else break; 

    _for(i,k,y[a].sz) z.pb(y[a][i]);
    _for(j,a+1,124) visit(y[j]) z.pb(i); 
    srt(z); visit(z) cout<<x[i];
}
