...
ll i,j,k;
ll t,n,a,b,c,m,l,r,p,q;
ll s,ans;
void dihan()
{
     cin>>n>>m>>k; vpii x,y,z,w; vi taken(n); 
    ...
    srt(w); srt(x); srt(y); srt(z);
    c=m;

    //cout<<w.sz<<" "<<x.sz<<" "<<y.sz<<" "<<z.sz<<endl;

    for(i=0;2*k>c&i<x.sz;i++,k--,c--)
    {
       taken[x[i].se]=2;
       s+=x[i].fi;     
    }
    ...
    else
    {
    	...
    	{
    		a=k;
    		for(i=0;a>0&i<z.sz;i++)
    	      { 

                if(taken[z[i].se]==1) a--;
                else if(!taken[z[i].se]) 
    			{
    				taken[z[i].se]=1;
    				s+=z[i].fi;
    				a--; c--;
    		 ...
    		 {
                outn(s); for(i=0,j=0;i<n&j<m;i++) if(taken[i]) {cout<<i+1<<" "; j++;}
    		 }
    		 }
    	}
    }
