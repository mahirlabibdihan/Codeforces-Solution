...
void dihan()
{
   cin>>n>>k; v_in(x,n); srt(x);

   a=lb(x,k); vll z(MAX);

   for(j=0;j<n;j++)
     	{ 
     		if(j+ceil(1.0*k/x[j])-1<n) {z[j]=1; j+=ceil(1.0*k/x[j])-1; c++; }
     		else {j=n; break;}
     	}

     	s=0;

   for_(i,MAX-1,-1) if(z[i]>0) z[i]+=s++;
     
     
   ans=n-a;  m=min((ll)ceil(1.0*k/x[0]),a);
   for(i=0;i<m;i++)
     {
     	for(c=0,j=i;j<a;j++)
     	{ 
     		if(z[j]>0) {c+=z[j]; j=n; break;}
     		eif(j+ceil(1.0*k/x[j])-1<n) {j+=ceil(1.0*k/x[j])-1; c++; }
     		else {j=n; break;}
     	}
