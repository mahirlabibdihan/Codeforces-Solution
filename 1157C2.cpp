...
void dihan()
{
    cin>>n; v_in(x,n); vector<char> z,y;

    a=0; b=n-1; c=0;
    while(a<=b)
    {
             if(x[a]>c&x[b]>c)
                 if(x[a]<x[b]) z.pb('L'),c=x[a++];          
                          else z.pb('R'),c=x[b--];
                   eif(x[a]>c) z.pb('L'),c=x[a++];   
                   eif(x[b]>c) z.pb('R'),c=x[b--];                 
                          else break;
    }
    ...
    {
             if(x[a]>c&x[b]>c)
                if(x[a]<=x[b]) y.pb('L'),c=x[a++];          
                          else y.pb('R'),c=x[b--]; 
                   eif(x[a]>c) y.pb('L'),c=x[a++];  
                   eif(x[b]>c) y.pb('R'),c=x[b--];                 
                          else break;
    }

    out(max(z.sz,y.sz)); 

    if(z.sz>y.sz) visit(z) cout<<i; 
    else visit(y) cout<<i; 
}
