...
void dihan()
{
   ...
   _for(i,0,n-1)
   {
   	 if(c==0&x[i]<x[i+1]) c=1;
   	 eif(c==1&x[i]>x[i+1]) c=2;
   }

   if(a==1|b==1|c==1) outn("NO");
   else 
   {
    _for(i,0,n-2)_for(j,i+1,n-1)if(x[j]>x[i])_for(k,j+1,n)if(x[k]<x[j])goto X; 
    X: (i<n-2)?cout<<"YES"<<" "<<i+1<<" "<<j+1<<" "<<k+1<<endl:cout<<"NO"<<endl;
   }
