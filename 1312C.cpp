...
#define rFOR(i,x) for(auto i=x.rbegin();i!=x.rend();i++)
...
ull t,n,i,j,k,a,b,c,m,l,r,p,q,s;
void dihan()
{
    cin>>n>>k; v_in(x,n); bit(60) b;

    _for(i,0,n)for(j=0;x[i]>0;j++,x[i]/=k)
      if(x[i]%k) if(b[j]|x[i]%k>1) goto X; else b[j]=1;
