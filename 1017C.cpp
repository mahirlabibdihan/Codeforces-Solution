...
{
    cin>>n; s=n;
    a=ceil(sqrt(1.0*n)); b=sqrt(1.0*n); vvi x(a);    
    _for(i,0,a)
    {
    	if(s%(a-i)==0&s/(a-i)==a) {x[i].pb(a); s-=a;}
    	else {x[i].pb(min(b,s)); s-=b;}
    }
    for(i=c=0;i<a;i++) _for(j,0,x[i][0]) x[i].pb(++c);
    for_(j,a-1,-1) _for(i,1,x[j][0]+1) cout<<x[j][i]<<" ";
}
