...
#define MAX 200001
ull t,n,i,j,k,a,b,c,d,m,l,r,p,q,s,ans;
void dihan()
{
    cin>>n; vp_in(x,n); 
    ...
    f(a,0,n-3)
    {
    	f(b,a+1,n-2)
    	{
    		p=pow(x[a].fi-x[b].fi,2)+pow(x[a].se-x[b].se,2);
    		f(c,b+1,n-1)
    		{
    			q=pow(x[a].fi-x[c].fi,2)+pow(x[a].se-x[c].se,2);
    			{
    				f(d,c+1,n)
    				{

    					r=pow(x[d].fi-x[b].fi,2)+pow(x[d].se-x[b].se,2);
    					s=pow(x[d].fi-x[c].fi,2)+pow(x[d].se-x[c].se,2);
    					if(q==s&p==r) ans++;
    				}
    			}
    		}
