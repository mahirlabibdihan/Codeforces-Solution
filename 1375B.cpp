...
#define MAX 200001
ll t,n,k,a,b,c,m,l,r,p,q,s;
int i,j;
void dihan()
{
    cin>>n>>m;  vvi x(n,vi (m)); c=0;


    _for(i,0,n)
      _for(j,0,m)
         {  cin>>x[i][j];
         	if((!i&(!j|j==m-1))|(i==n-1&(!j|j==m-1))) {if(x[i][j]<3)  x[i][j]=2; else c=1;}

         	else if(!i|!j|i==n-1|j==m-1) {if(x[i][j]<4) x[i][j]=3; else c=1;}

         	else if(x[i][j]<5) x[i][j]=4; else c=1;
         }
