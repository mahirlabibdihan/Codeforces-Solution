...
ll t,n,i,j,k,a,b,c,d,m,l,r,p,q,s,ans; vvll adj(100001); vector<bool> visited(100001);
...
#define MAX 100001
vll root(MAX),value(MAX),y(MAX);
void dfs(int node)
{
   if(visited[node]) return;
   visited[node]=true;
   ans=min(ans,value[node]);
...
{
  cin>>n>>m; 
  f(i,1,n+1) cin>>value[i]; input(m);
  f(i,1,n+1) if(!visited[i]) ans=INF,dfs(i),s+=ans;
  outn(s);
}
int main(){____ dihan();}
