...
void dihan()
{
    ...
    for(i=0;i<n-1;i++)
        if(x[i]<x[i+1]) {s=1; br;}
        eif(x[i]>x[i+1]) {s=0; br;}
    if(s)
    {
        for(i=0;i<n-1&x[i]<=x[i+1];i++);
        if(i==n-1) printf("yes\n1 1");
        else{for(j=i+1;j<n-1&x[j]>=x[j+1];j++);
            if(j==n-1&&x[j]>=x[i-1]) printf("yes\n%d %d",i+1,n);
            eif(x[j+1]>x[i]) {for(k=j+1;k<n-1&x[k]<=x[k+1];k++);
                 (k==n-1&&x[j+1]>=x[i+1])?printf("yes\n%d %d",i+1,j+1):puts("no");}         
            else puts("no");}   
    }
    eif(!s)
    {
