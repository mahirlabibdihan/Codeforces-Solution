...
void vs_out(string x){ for(auto i:x) cout<<i<<endl;}
...
template<typename T> void adjdif(vector<T>&x){ll n=x.size(),i;for(i=0;i<n-1;i++) x[i]-=x[i+1];}
