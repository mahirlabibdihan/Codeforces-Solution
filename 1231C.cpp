      ...
      for_(j,m-1,-1)
        {
          if(!x[i][j])
          {
              if(i==n-1) x[i][j]=x[i][j+1]-1;
              eif(j==m-1) x[i][j]=x[i+1][j]-1;
              else x[i][j]=min(x[i][j+1],x[i+1][j])-1;
          }          
          if(i==0&j==0) s+=x[i][j];
          eif(i==0) {if(x[i][j]>x[i][j-1]) s+=x[i][j]; }
          eif(j==0) {if(x[i][j]>x[i-1][j]) s+=x[i][j];}
          eif(x[i][j]>x[i-1][j]&x[i][j]>x[i][j-1]) s+=x[i][j];
          else goto X;
        }
      X: (i>-1)?outn(-1):outn(s);
}
