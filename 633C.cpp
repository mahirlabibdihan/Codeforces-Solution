...
#define revr(x) reverse(x.begin(),x.end())
...
void dihan()
{
   cin>>n; str_in(x); revr(x);
   cin>>m; vs_in(y,m); sort(rptr(y)); 
   ...
   for(i=0;i<n;i++)
   {
   	 for(j=0;j<m&i<n;j++)
   	 {
   	 	if(x[i]==y[j][i-s]|x[i]==y[j][i-s]+32){ j--; i++;}

   	 	eif(i-s==y[j].sz) {ans.pb(y[j]); i--; s+=y[j].sz; break;}
   	 }
   }
