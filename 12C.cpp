...
#define v_in(x,n) vi x(n); for(auto &i:x) cin>>i
...
void dihan()
{
	string fruit;  u_m<string,int> map; vi y;
    cin>>n>>m; v_in(x,n); srt(x);
    for(i=0;i<m;map[fruit]++,i++) cin>>fruit;    	    
    visit(map) y.pb(i.se); rsrt(y);
    for(i=0;i<y.sz;i++)  a+=(y[i]*x[i]); 
    for(i=0,j=n-1;i<y.sz;i++,j--) b+=(y[i]*x[j]);
    out2(a,b);
}
