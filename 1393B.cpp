...
if(q==0)
   for(auto i:x)
   {
   	 if(i.se>=2)
   	 {
   	 	if(i.fi==p)
   	 	{
   	 		if(i.se>=6) {q=i.fi; if(x[q]<8&q==r)r=0; break;}
   	 	}
   	 	else {q=i.fi;if(x[p]<4&q==r)r=0; break;}
   	 }
   }
...
eif(y=='-'&b==r)
{
   ...
   if(p>0&q>0&r>0) out("YES"); else out("NO");
   }
}
int main(){____ dihan();}